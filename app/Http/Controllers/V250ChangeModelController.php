        ...
        try {
            DB::transaction(function () use ($request) {
                ...
                foreach ($oldIdentityCodes as $oldIdentityCode) {
                    $oldRemark[$oldIdentityCode] = '设备：' . $oldIdentityCode . '，' . '在换型任务中被使用。详情：工区：' . session('account.work_area');
                }
                foreach ($newIdentityCodes as $oldIdentityCode) {
                    $newRemark[$oldIdentityCode] = '设备：' . $oldIdentityCode . '，' . '在换型任务中被使用。详情：工区：' . session('account.work_area');
                }
                # 设备加锁
        ...
        try {
            $functions = [
                'CHANGE_MODEL' => function () use ($request, $sn) {
                    ...
                    if ($un_out_entire_instance_count > 0) return JsonResponseFacade::errorForbidden("当前任务存在未完成出所的设备：{$un_out_entire_instance_count}台。不能交付");
