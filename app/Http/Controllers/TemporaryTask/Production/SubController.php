                ...
                return DB::transaction(function () use ($request, &$ret) {
                    ...
                    use (
                        ...
                        foreach ($entire_instances as $entire_instance) {
                            ...
                            $entire_instance_locks[$entire_instance->identity_code] = "设备：{$entire_instance->identity_code}，在高频修入所中被使用。详情，高频修任务：{$high_frequency_order_in_sn}";
                        }
                        # 入所设备上锁
                    ...
                    $in = function () use ($request) {
                        ...
                        try {
                            $lock_ret = EntireInstanceLock::setOnlyLock(
                                ...
                                ['NEW_STATION'],
                                "设备：{$request->get('identityCode')}，在新站入所任务中被使用。详情：新站入所任务{$request->get('serialNumber')}",
            ...
            return DB::transaction(function () use ($request) {
                $high_frequency = function () use ($request) {
                    ...
                    EntireInstanceLock::setOnlyLock(
                        ...
                        ['HIGH_FREQUENCY'],
                        "设备：{$usable_entire_instance}，在高频修出所中被使用。详情：高频修{$request->get('outSn')}",
                    ...
                    foreach ($old_entire_instances as $model_name => $entire_instances) {
                        foreach ($entire_instances as $entire_instance) {
                            if ($usable_entire_instances->get($entire_instance->model_name)) {
                                ...
                                $entire_instance_locks[$usable_entire_instance] = "设备：{$usable_entire_instance}，在高频修出所中被使用。详情：高频修{$out_order->serial_number}";
                            }
                        }
                    }
                        ...
                        EntireInstanceLock::freeLock(
                            ...
                            function () use ($request, $old_entire_instance) {
                                ...
                                EntireInstanceLock::setOnlyLock(
                                    ...
                                    ['HIGH_FREQUENCY'],
                                    "设备：{$request->get('newIdentityCode')}，在高频修出所中被使用。详情：高频修{$request->get('outSn')}",
                                    function () use ($request, $old_entire_instance) {
