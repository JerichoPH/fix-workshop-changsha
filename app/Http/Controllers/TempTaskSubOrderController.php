...
7. 交付";
    ...
    final public function postBindEntireInstance(Request $request)
    {
        try {
            ...
            if (!$newEntireInstance) return JsonResponseFacade::errorEmpty("成品设备：{$request->get('newIdentityCode')}不存在");
    ...
    )
    {
        ...
        $lock = EntireInstanceLock::setOnlyLock(
            ...
            [strtoupper($lockName)],
            "设备：{$newEntireInstance->identity_code}在临时生产任务中被使用；详情：[{$tempTaskSerialNumber}]{$tempTaskTitle}"
