...
class SendRepairController extends Controller
{
    ...
    final public function create()
    {
        try {
            ...
            $file = FileSystem::init(storage_path('app/basicInfo'));
            $maintains = $file->join('stations.json')->fromJson();

            return view('Storehouse.SendRepair.create', [
                'factories' => $factories,
                'maintains' => $maintains,
                'maintainJson' => TextHelper::toJson($maintains),
                'firstSceneWorkshopUniqueCode' => env('CURRENT_WORKSHOP_UNIQUE_CODE', ''),
            ]);
        } catch (\Exception $exception) {
            return back()->with('danger', $exception->getMessage());
        }
    ...
    final public function store()
    {
        try {
            ...
            if ($tmpMaterials->isEmpty()) return back()->with('danger', '请选择送修设备');
            $from_station_code = request('from_station_code');
            $from_scene_workshop_code = request('from_scene_workshop_code');
            if (empty($from_scene_workshop_code)) return back()->with('danger', '送修单位不能为空');
            if (empty($from_station_code)) {
                $from_code = $from_scene_workshop_code;
                $maintain = DB::table('maintains')->where('unique_code', $from_scene_workshop_code)->first();
                $from_code_name = empty($maintain) ? '' : $maintain->name;
                $type_prefix = 'W';
            } else {
                $from_code = $from_station_code;
                $maintain = DB::table('maintains')->where('unique_code', $from_station_code)->first();
                $from_code_name = empty($maintain) ? '' : $maintain->name;
                $type_prefix = 'S';
            }

            $to_check = request('to_check');
            $to_scene_workshop_code = request('to_scene_workshop_code');
            $to_factory_unique_code = request('to_factory_unique_code');
            switch ($to_check) {
                case 'sceneWorkshop':
                    $to_code = $to_scene_workshop_code;
                    $maintain = DB::table('maintains')->where('unique_code', $to_scene_workshop_code)->first();
                    $to_code_name = empty($maintain) ? '' : $maintain->name;
                    $type_suffix = empty($to_scene_workshop_code) ? '' : 'W';
                    break;
                case 'factory':
                    $to_code = $to_factory_unique_code;
                    $factory = DB::table('factories')->where('unique_code', $to_factory_unique_code)->first();
                    $to_code_name = empty($factory) ? '' : $factory->name;
                    $type_suffix = empty($to_factory_unique_code) ? '' : 'F';
                    break;
                default:
                    $to_code = '';
                    $type_suffix = '';
            ...
            if (empty($to_code)) return back()->with('danger', '接收单位不能为空');
            $type = $type_prefix . 'T' . $type_suffix;

            $sendRepairUniqueCode = DB::transaction(function () use ($tmpMaterials, $from_code, $to_code, $type, $from_code_name, $to_code_name) {
                ...
                $tmpBreakdownLogs = DB::table('tmp_breakdown_logs as tbl')
                    ...
                    ->where('tbl.account_id', session('account.id'))->get();
                ...
                foreach ($tmpMaterials as $tmpMaterial) {
                    ...
                    $description = "送修单位：{$from_code_name}；接收单位：{$to_code_name}；";
                    ...
                    EntireInstanceLogFacade::makeOne('送修', $tmpMaterial->material_unique_code, 0, "/storehouse/sendRepair/{$sendRepairUniqueCode}", $description, $tmpMaterial->material_type['value']);
                    ...
                    if (array_key_exists($tmpMaterial->material_unique_code, $breakdownTypeWithMaterials)) {
                        ...
                        if (!empty($breakdownLogId)) {
                            ...
                            EntireInstanceLogFacade::makeOne('送修故障', $tmpMaterial->material_unique_code, 0, "", $description, $tmpMaterial->material_type['value']);
                        }
                    }
            ...
            return redirect(url('storehouse/sendRepair', $sendRepairUniqueCode));
    ...
    final public function show($uniqueCode)
    {
        try {
            $sendRepair = SendRepair::with([
                ...
                'WithToFactory',
                'WithToMaintain'
            ])->where('unique_code', $uniqueCode)->firstOrFail();
        ...
        try {
            ...
            EntireInstanceLock::freeLock(
                ...
                function () use ($sendRepairInstance, $materialUniqueCode) {
                    ...
                    $sendRepair->saveOrFail();
                    # 日志
                    EntireInstanceLogFacade::makeOne('验收', $materialUniqueCode, 0, "", '送修验收', $sendRepairInstance->material_type);
                }
            );
    ...
    final public function tmpMaterialWithStore()
    {
        try {
            ...
            EntireInstanceLock::setOnlyLock(
                $identityCode,
                [$this->_sign],
                "{$materialTypeName}设备：{$identityCode}，在送修中被使用。详情：送修操作人员：" . session('account.account'),
