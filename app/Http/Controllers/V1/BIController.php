...
class BIController extends Controller
{
    ...
    final public function getFacilityAndOverdueStatistics()
    {
        try {
            ...
            $total_statistics = EntireInstanceSerializer::INS([
                ...
                ->get();
            $total_statistics->each(function ($total_statistic) use (&$statistics) {
                if (!@$statistics[$total_statistic->category_unique_code]) $statistics[$total_statistic->category_unique_code] = ['name' => $total_statistic->category_name, 'total' => 0, 'overdue' => 0, 'storehouse' => 0,];
            ...
            use (&$statistics) {
                if (!@$statistics[$overdue_statistic->category_unique_code]) $statistics[$overdue_statistic->category_unique_code] = ['name' => $overdue_statistic->category_name, 'total' => 0, 'overdue' => 0, 'storehouse' => 0,];
                $statistics[$overdue_statistic->category_unique_code]['overdue'] += $overdue_statistic->aggregate;
            });

            // 仓库统计
            $storehouse_statistics = EntireInstanceSerializer::INS([
                EntireInstanceSerializer::$IS_PART => false,
            ])
                ->generateQueryRelationShipS()
                ->selectRaw(implode(',', [
                ...
                ->where('ei.location_unique_code', 'like', request('location_unique_code') . '%')
                ->where('c.unique_code', 'like', 'S%')
                ->get();
            $storehouse_statistics->each(function ($storehouse_statistic) use (&$statistics) {
                if (!@$statistics[$storehouse_statistic->category_unique_code]) $statistics[$storehouse_statistic->category_unique_code] = ['name' => $storehouse_statistic->category_name, 'total' => 0, 'overdue' => 0, 'storehouse' => 0,];
                $statistics[$storehouse_statistic->category_unique_code]['storehouse'] = $statistics[$storehouse_statistic->category_unique_code]['storehouse'] += $storehouse_statistic->aggregate;
            });
    ...
    final public function getEquipmentAndOverdueStatistics()
    {
        try {
            ...
            $overdue_statistics = EntireInstanceSerializer::INS([
                ...
                ->get();
            $overdue_statistics->each(function ($overdue_statistic) use (&$statistics) {
                if (!@$statistics[$overdue_statistic->category_unique_code]) $statistics[$overdue_statistic->category_unique_code] = ['name' => $overdue_statistic->category_name, 'total' => 0, 'overdue' => 0, 'storehouse' => 0,];
                $statistics[$overdue_statistic->category_unique_code]['overdue'] += $overdue_statistic->aggregate;
            });
    ...
    final public function getEquipmentStatusStatistics()
    {
        try {
            ...
            $status_statistics->each(function ($status_statistic) use (&$statistics) {
                $num_status = collect(EntireInstance::$STATUSES_FOR_BI)->get($status_statistic->status);
     ...
     * @return JsonResponse
     */
    final public function getWarehouseStatisticsBy7DaysForEmergency(): JsonResponse
    {
        $dates = [];
        $directions = [
            'IN' => '入所',
        ...
        $origin_time = array_first($dates) . ' 00:00:00';
        $finish_time = array_last($dates) . ' 23:59:59';

        // 获取应急备品中心人员
        $account_ids = DB::table('accounts as a')
            ->select('a.id')
            ->join(DB::raw('work_areas wa'), 'a.work_area_unique_code', '=', 'wa.unique_code')
            ->where('wa.type', 'emergency')
            ->get()
            ->pluck('id');
        ...
        // 查询入库数据
        $tmp_for_in = DB::table('warehouse_report_entire_instances as wrei')
            ->selectRaw(implode(',', [
                'count(c.name) as aggregate',
                'c.name as category_name',
                "DATE_FORMAT(wr.created_at,'%Y-%m-%d') as time",
            ]))
            ->join(DB::raw('warehouse_reports wr'), 'wrei.warehouse_report_serial_number', '=', 'wr.serial_number')
            ->join(DB::raw('entire_instances ei'), 'wrei.entire_instance_identity_code', '=', 'ei.identity_code')
            ->join(DB::raw('categories c'), 'ei.category_unique_code', '=', 'c.unique_code')
            ->where('wr.direction', 'IN')
            // ->whereIn('wrei.processor_id', $account_ids->toArray())
            ->whereBetween('wr.updated_at', [$origin_time, $finish_time])
            ->groupBy(['category_name', 'time'])
            ->get();

        // 组合种类名称(入库)
        $tmp_for_in->groupBy('category_name')->each(function ($item, $category_name) use (&$warehousing_legends) {
            $warehousing_legends[] = $category_name . '(入库)';
        });

        // 组合统计数据(入库)
        $tmp_for_in->groupBy('time')
            ->each(function ($data, $time) use (&$statistics) {
                if (!array_key_exists($time, $statistics)) $statistics[$time] = [];
                $data->each(function ($datum) use (&$statistics, $time) {
                    if (!array_key_exists($datum->category_name, $statistics[$time])) $statistics[$time][$datum->category_name . '(入库)'] = 0;
                    $statistics[$time][$datum->category_name . '(入库)'] += $datum->aggregate;
                });
            });

        // 统计出库数据
        $tmp_for_out = DB::table('warehouse_report_entire_instances as wrei')
            ->selectRaw(implode(',', [
                'count(c.name) as aggregate',
                'c.name as category_name',
                "DATE_FORMAT(wr.created_at,'%Y-%m-%d') as time",
        ...
        // 组合种类名称(出库)
        $tmp_for_out->groupBy('category_name')->each(function ($item, $category_name) use (&$warehousing_legends) {
            $warehousing_legends[] = $category_name . '(出库)';
        });

        // 组合统计数据(出库)
        $tmp_for_out->groupBy('time')
            ->each(function ($data, $time) use (&$statistics) {
                if (!array_key_exists($time, $statistics)) $statistics[$time] = [];
                $data->each(function ($datum) use (&$statistics, $time) {
                    if (!array_key_exists($datum->category_name, $statistics[$time])) $statistics[$time][$datum->category_name . '(出库)'] = 0;
                    $statistics[$time][$datum->category_name . '(出库)'] += $datum->aggregate;
                });
            });

        return JsonResponseFacade::dict([
            'warehousing_axis' => $dates,
            'warehousing_legends' => $warehousing_legends,
