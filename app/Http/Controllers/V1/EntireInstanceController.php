...
//                    if (!$ei) return HttpResponseHelper::errorEmpty('未查询到设备');
...
//        }
        ...
        $entireInstance = DB::table('entire_instances as ei')
            ...
            ->first([
                ...
                'ei.last_installed_time as installed_time',
        ...
        if (empty($entireInstance)) return HttpResponseHelper::errorEmpty('设备编号不存在');
    ...
    final public function bindingRfidTid(Request $request, string $identityCode)
    {
        try {
            ...
            EntireInstanceLogFacade::makeOne('绑定RFID TID', $entireInstance->identity_code, 3, '', $request->get('rfid_tid'));

            return HttpResponseHelper::created('绑定成功');
        } catch (ModelNotFoundException $e) {
            return HttpResponseHelper::errorEmpty('设备不存在');
        } catch (\Exception $e) {
    ...
    final public function install()
    {
        ...
        foreach ($request as $item) {
            if ($item['old_rfid_tid']) {
                # 替换设备上道
                if (!array_key_exists('old_rfid_tid', $item)) return HttpResponseHelper::errorEmpty('旧设备编号不能为空');
                if (!array_key_exists('new_rfid_tid', $item)) return HttpResponseHelper::errorEmpty('新设备编号不能为空');
                ...
                if (!$old_entire_instance) return HttpResponseHelper::errorEmpty('旧设备不存在');
                ...
                if (!$new_entire_instance) return HttpResponseHelper::errorEmpty('新设备不存在');
                ...
                if (!$new_entire_instance) return HttpResponseHelper::errorEmpty('设备不存在');
                //获取车间名称
                $parent_unique_code = DB::table('maintains')->where('name', $item['maintain_station_name'])->value('parent_unique_code');
                $maintain_workshop_name = DB::table('maintains')->where('unique_code', $parent_unique_code)->value('name');
...
//                $new_entire_instance = DB::table('entire_instances')->where('identity_code', $item['new_rfid_tid'])->orWhere('serial_number', $item['new_rfid_tid'])->first();
    ...
    final public function uninstall()
    {
        ...
        foreach ($request as $code) {
            switch (strlen($code)) {
                default:
                    $entireInstance = DB::table('entire_instances')->where('deleted_at', null)->where('serial_number', $code)->first(['identity_code', 'status']);
                ...
                case 14:
                case 19:
                    $entireInstance = DB::table('entire_instances')->where('deleted_at', null)->where('identity_code', $code)->first(['identity_code', 'status']);
                    if ($entireInstance == null) {
                        $noExists[] = $code;
                        continue;
                    }
                ...
                case 24:
                    ...
                    if ($firstCode == '130E' && $firstCode == '130F') {
                        $entireInstance = DB::table('entire_instances')->where('deleted_at', null)->where('identity_code', CodeFacade::hexToIdentityCode($code))->first(['identity_code', 'status']);
                        if ($entireInstance == null) {
                            $noExists[] = $code;
                            continue;
                        }
                        ...
                        $entireInstance = DB::table('entire_instances')->where('deleted_at', null)->where('rfid_code', $code)->first(['identity_code', 'status']);
                        if ($entireInstance == null) {
                            $noExists[] = $code;
                            continue;
                        }
