...
$api->version(['v1', 'v2'], function ($api) {
    ...
    $api->group(['prefix' => 'pda', 'namespace' => 'App\Http\Controllers\V1'], function ($api) {
        ...
        $api->get('entireInstanceByIdentityCode/{identityCode}', 'PDAController@getEntireInstance')->name('api.pda.getEntireInstance');  // 根据唯一编号获取设备详情
    ...
    $api->group(['prefix' => 'pda', 'namespace' => 'App\Http\Controllers\V1', 'middleware' => 'api-check-jwt'], function ($api) {
        ...
        // 搜索->上道
        $api->post('installOfSearch', 'PDAController@postInstallOfSearch')->name('api.pda.postInstallOfSearch');  // 搜索->上道
        // 上道
        $api->post('install', 'PDAController@postInstall')->name('api.pda.postInstall');  // 上道

        // 搜索->下道
        $api->post('uninstallOfSearch', 'PDAController@postUninstallOfSearch')->name('api.pda.postUninstallOfSearch');  // 搜索->下道
        // 下道
        $api->post('uninstall', 'PDAController@postUninstall')->name('api.pda.postUninstall');  // 下道

        // 搜索->现场备品入库
        $api->post('sceneWarehouseInOfSearch', 'PDAController@postSceneWarehouseInOfSearch')->name('api.pda.postSceneWarehouseInOfSearch');  // 搜索->现场备品入库

        // 现场备品入库
        $api->post('sceneWarehouseIn', 'PDAController@postSceneWarehouseIn')->name('api.pda.postSceneWarehouseIn');  // 现场备品入库
    ...
    $api->group(['prefix' => 'HH'], function ($api) {
        // 自由访问
        $api->get('entireInstancesByStationName', 'App\Http\Controllers\V1\HHController@getEntireInstancesByStationName')->name('HH.api1.entireInstancesByStationName.get');  // api1
        $api->post('fixWorkflowsByEIID', 'App\Http\Controllers\V1\HHController@postFixWorkflowsByEIID')->name('HH.fixWorkflowsByEIID.post');  // api2
        $api->get('recodesByProcessSN/{fix_workflow_process_sn}', 'App\Http\Controllers\V1\HHController@getRecodesByProcessSN')->name('HH.recodesByProcessSN.get');  // api3
        $api->post('breakdownExplain', 'App\Http\Controllers\V1\HHController@postBreakdownExplain')->name('HH.breakdownExplain.post');  // api4
        $api->post('entireInstanceLogsByEIID', 'App\Http\Controllers\V1\HHController@postEntireInstanceLogsByEIID')->name('HH.entireInstanceLogsByEIID.post');  // api5
        $api->get('entireInstance/{identityCode}', 'App\Http\Controllers\V1\HHController@getEntireInstance')->name('HH.entireInstance.get');  // 测试1
    });
    ...
    // stand detector store
    $api->group(['prefix' => 'detector'], function ($api) {
    ...
    $api->group([
        ...
        $api->get('warehouseStatisticsBy7DaysForEmergency','BIController@getWarehouseStatisticsBy7DaysForEmergency')->name('api.bi.getWarehouseStatisticsBy7DaysForEmergency');  // 获取7天应急备品工区人员出入所统计
    });
});
