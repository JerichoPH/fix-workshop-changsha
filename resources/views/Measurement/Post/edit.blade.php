    ...
    <section class="content">
        <div class="box box-warning">
                        ...
                        <div class="col-sm-10 col-md-8">
                            <div class="input-group">
                                ...
                                <div class="input-group-addon">
                                    <label style="font-weight: normal;"><input type="checkbox" name="is_extra_tag" {{$measurement->is_extra_tag ? 'checked' : ''}} value="1">使用额外测试项</label>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="box-footer">
                <a href="{{url('measurements')}}?page={{request('page',1)}}" class="btn btn-default pull-left btn-flat"><i class="fa fa-arrow-left">&nbsp;</i>返回</a>
        ...
        fnUpdate = function () {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    alert(response);
                    location.href = "{{url('measurements')}}?page{{request('page',1)}}";
                },
        ...
        fnGetWarehouseProductPartByWarehouseProductId = function (entireModelUniqueCode) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    html = '';
                    for (let key in response) {
                        console.log(response[key].extra_tag);
                        html += `<option value=${response[key].extra_tag} ${response[key].extra_tag === "{{$measurement->extra_tag}}" ? 'selected' : ''}>${response[key].extra_tag}</option>`;
                    }
