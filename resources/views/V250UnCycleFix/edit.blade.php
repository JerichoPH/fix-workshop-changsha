...
@section('content')
    ...
    <section class="content">
                        ...
                        <div class="col-md-12">
                            <div class="pull-right">
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ url('v250UnCycleFix') }}?page={{ request('page',1) }}&type={{ request('type')}}" class="btn btn-default btn-flat">返回任务列表</a>
                                    <a {{ $taskOrder->status['code'] == 'DONE' ? 'disabled' : '' }} href="javascript:" onclick="$('#modalUnCycleFix').modal('show')" class="btn btn-default btn-flat {{ $taskOrder->status['code'] == 'DONE' ? 'disabled' : '' }}">添加设备</a>
        ...
        <!--状态修-->
        <div class="modal fade" id="modalUnCycleFix">
            <div class="modal-dialog modal-dialog-centered" style="width:80vw;height:90vh">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title">状态修</h4>
                    </div>
                    <div class="modal-body form-horizontal">
                        <iframe src="{{ url('v250UnCycleFix') }}/unCycleFixList?sn={{ $taskOrder->serial_number }}&stationName={{ $taskOrder->MaintainStation->name }}&type=1&is_iframe=1" style="width:calc(80vw - 30px);height: calc(90vh - 95px);border:none;margin:auto;"></iframe>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default btn-flat pull-left btn-sm" data-dismiss="modal"><i class="fa fa-times">&nbsp;</i>关闭</button>
                    </div>
...
@section('script')
    <script>
        ...
        $(function () {
            // 关闭iframe时刷新
            $('#modalUnCycleFix').on('hidden.bs.modal', function (e) {
                location.reload();
            });
