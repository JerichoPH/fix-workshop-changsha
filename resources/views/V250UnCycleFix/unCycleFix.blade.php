                        ...
                        <div class="layui-tab layui-steps layui-steps-readonly" lay-filter="stepsDemoForget" style="max-width: 100%;">
                            <ul class="layui-tab-title">
                                <li class="{{ request('type') == '1' ? "layui-this" : '' }}">
                                    ...
                                    <span class="layui-steps-content">选择现场超期设备</span>
                                </li>
                                ...
                                <li class="{{ request('type') == '3' ? "layui-this" : '' }}">
                                    ...
                                    <span class="layui-steps-content">更换完成</span>
                                </li>
                            </ul>
                                                ...
                                                <table class="table table-hover table-striped table-condensed" id="table">
                                                    ...
                                                    @foreach($workshopEntireInstances as $workshopEntireInstance)
                                                        <tr>
                                                            ...
                                                            <input type="hidden" id="{{ $workshopEntireInstance->identity_code }}" value="{{ $workshopEntireInstance->model_name }}">
                                                ...
                                                <table class="table table-hover table-striped table-condensed" id="table">
                                                    ...
                                                    @foreach($workshopEntireInstances1 as $workshopEntireInstance1)
                                                        <tr>
                                                            ...
                                                            <input type="hidden" id="{{ $workshopEntireInstance1->identity_code }}" value="{{ $workshopEntireInstance1->model_name }}">
...
{{--                                            @if($workshopEntireInstances1->hasPages())--}}
{{--                                                <div class="box-footer">--}}
{{--                                                    {{ $workshopEntireInstances1->appends(request()->all())->links() }}--}}
{{--                                                </div>--}}
{{--                                            @endif--}}
                                        </div>
                                    </div>
                                ...
                                <div class="{{ request('type') == '3' ? 'layui-tab-item text-center layui-show' : 'layui-tab-item text-center' }}" style="padding: 40px 150px 10px 60px;">
                                    ...
                                    <div style="text-align: center;margin: 50px 0 25px 0;">
                                        <a href="{{ url('v250UnCycleFix') }}/unCycleFixList?sn={{ request('sn') }}&stationName={{ request('stationName') }}&type=1&is_iframe=1" class="layui-btn layui-btn-radius" lay-filter="stepDemoFormSubmit3" lay-submit>再次换型</a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('script')
    ...
    <script>
        ...
        function fnQuery(sn) {
            ...
            location.href = `{{ url('v250UnCycleFix') }}/unCycleFixList?sn=${sn}&categoryUniqueCode=${categoryUniqueCode}&entireModelUniqueCode=${entireModelUniqueCode}&subModelUniqueCode=${subModelUniqueCode}&factoryName=${factoryName}&entireInstanceUniqueCode=${entireInstanceUniqueCode}&stationName=${stationName}&type=${type}&is_iframe=1`;
        }
        ...
        layui.config({
            ...
            form.on('submit(stepDemoFormSubmit1)', function () {
                ...
                $("input[type='checkbox'][name='check']:checked").each((index, item) => {
                    ...
                    if (oldModelName.hasOwnProperty($('#'+ new_code).val())) {
                        oldModelName[$('#'+ new_code).val()] += 1;
                    }else {
                        oldModelName[$('#'+ new_code).val()] = 1;
                    }
                });
                ...
                var sn = '{{ request('sn') }}';
                var stationName = '{{ request('stationName') }}';
                var oldModelNameJson = {};
                for (var key in oldModelName) {
                    oldModelNameJson[key] = oldModelName[key];
                }
                oldModelNameJson = JSON.stringify(oldModelNameJson);

                admin.btnLoading('[lay-filter="stepDemoFormSubmit1"]');  // 加载动画
                setTimeout(function () {
                    location.href = `{{ url('v250UnCycleFix') }}/unCycleFixList?sn=${sn}&stationName=${stationName}&oldIdentityCodes=${oldIdentityCodes}&oldModelNameJson=${oldModelNameJson}&type=2&is_iframe=1`;
                }, 600);
                return false;
            });
            ...
            form.on('submit(stepDemoFormSubmit2)', function () {
                ...
                var oldModelName = urlinfo.split("?")[1].split("&")[3].split("=")[1];
                var modelName = oldModelName.replaceAll('%22', '');
                return false;
                ...
                $("input[type='checkbox'][name='check1']:checked").each((index, item) => {
                    ...
                    if (newModelName.hasOwnProperty($('#'+ new_code).val())) {
                        newModelName[$('#'+ new_code).val()] += 1;
                    }else {
                        newModelName[$('#'+ new_code).val()] = 1;
                    }
                });
                ...
                // 判断数量
                for (let oldKey in oldModelName) {
                    for (let newKey in newModelName) {
                        if (oldKey == newKey) {
                            if (oldModelName > newModelName) {
                                layer.msg('请再勾选型号为' + oldKey + '的' + (oldModelName - newModelName) + '个设备', {icon: 7});
                            }else if (oldModelName < newModelName) {
                                layer.msg('请取消勾选型号为' + oldKey + '的' + (newModelName - oldModelName) + '个设备', {icon: 7});
                            }
                        }
                    }
                }
                if (oldModelName.length != newModelName.length) {
                    layer.msg('超期设备与更换设备数量不一致', {icon: 7});
                }
