            ...
            <div class="box box-solid">
                <div class="box-header">
                    ...
                    <div class="pull-right btn-group btn-group-sm">
                        <a href="{{ url('v250TaskOrder/create') }}?type={{ request('type') }}" class="btn btn-flat btn-success">新建</a>
                    </div>
                </div>
                <div class="box-body">
                    <div class="table-responsive">
                        <table class="table table-hover table-striped table-condensed" id="table">
                            ...
                            @foreach($taskOrders as $taskOrder)
                                <tr>
                                    <td>
                                        <a href="{{ url('v250TaskOrder',$taskOrder->serial_number) }}/edit?page={{ request('page', 1) }}&type={{ request('type')}}" class="label label-primary">
    ...
    <script>
        ...
        function fnDelete(sn) {
            if (confirm('删除不能恢复，是否确认'))
                $.ajax({
                    url: `{{ url('v250TaskOrder') }}/${sn}`,
                    type: 'delete',
                    data: {id: sn},
                    success: function (res) {
                        console.log(`{{ url('v250TaskOrder')}}/${sn} success:`, res);
                        alert(res.msg);
                        location.reload();
                    },
                    error: function (err) {
                        console.log(`{{ url('v250TaskOrder')}}/${sn} fail:`, err);
