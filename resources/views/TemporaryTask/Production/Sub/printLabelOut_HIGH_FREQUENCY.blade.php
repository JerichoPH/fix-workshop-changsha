...
{{--            <li><a href="{{ url('temporaryTask/production/sub',$out_sn) }}?type=HIGH_FREQUENCY&direction=OUT">高频修出所计划详情</a></li>--}}
    ...
    <section class="content">
        ...
        <div class="box box-solid">
            <div class="box-header">
                <h3 class="box-title">高频修出所设备列表</h3>
                <!--右侧最小化按钮-->
                <div class="pull-right btn-group btn-group-sm">
{{--                    <a href="{{ url('temporaryTask/production/sub',$out_sn) }}?type=HIGH_FREQUENCY&direction=OUT" class="btn btn-flat btn-default"><i class="fa fa-arrow-left">&nbsp;</i>返回</a>--}}
...
@section('script')
    <script>
        ...
        function fnAutoBindEntireInstances() {
            $.ajax({
                url: `{{ url('temporaryTask/production/sub/autoBindEntireInstances') }}`,
                type: 'post',
                data: {
                    type: 'HIGH_FREQUENCY',
                    outSn: '{{ $out_sn }}',
                ...
                success: function (res) {
                    console.log(`{{ url('temporaryTask/production/sub/autoBindEntireInstances') }} success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('temporaryTask/production/sub/autoBindEntireInstances') }} fail:`, err);
        ...
        function fnDeleteBindEntireInstances() {
            $.ajax({
                url: `{{ url('temporaryTask/production/sub/bindEntireInstances') }}`,
                type: 'delete',
                data: {
                    type: 'HIGH_FREQUENCY',
                    outSn: '{{ $out_sn }}'
                ...
                async: false,
                success: function (res) {
                    console.log(`{{ url('temporaryTask/production/sub/bindEntireInstances') }} success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('temporaryTask/production/sub/bindEntireInstances') }} fail:`, err);
                    if (err.status === 401) location.href = "{{ url('login') }}";
                    alert(err['responseJSON']['message']);
                }
            });
        ...
        function fnBindEntireInstance(oldIdentityCode, newIdentityCode) {
            if (newIdentityCode) {
                $.ajax({
                    url: `{{ url('temporaryTask/production/sub/bindEntireInstance') }}`,
                    type: 'post',
                    data: {
                        type: 'HIGH_FREQUENCY',
                        oldIdentityCode,
                    ...
                    async: false,
                    success: function (res) {
                        console.log(`{{ url('temporaryTask/production/sub/bindEntireInstance') }} success:`, res);
                        location.reload();
                    },
                    error: function (err) {
                        console.log(`{{ url('temporaryTask/production/sub/bindEntireInstance') }} fail:`, err);
                        if (err.status === 401) location.href = "{{ url('login') }}";
                        alert(err['responseJSON']['message']);
                    }
                });
                ...
                $.ajax({
                    url: `{{ url('repairBase/highFrequencyOrder/bindEntireInstance') }}`,
                    type: 'delete',
                    data: {
                        type: 'HIGH_FREQUENCY',
                        oldIdentityCode,
                    ...
                    async: false,
                    success: function (res) {
                        console.log(`{{ url('repairBase/highFrequencyOrder/bindEntireInstance') }} success:`, res);
                        location.reload();
                    },
                    error: function (err) {
                        console.log(`{{ url('repairBase/highFrequencyOrder/bindEntireInstance') }} fail:`, err);
                        if (err.status === 401) location.href = "{{ url('login') }}";
                        alert(err['responseJSON']['message']);
                    }
                });
        ...
        $('.select-bind-entire-instance').on('change', function () {
            ...
            if ($(this).is(':checked')) {
                // 绑定
                $.ajax({
                    url: `{{ url('temporaryTask/production/sub/autoBindEntireInstance') }}`,
                    type: 'post',
                    data: {
                        type: 'HIGH_FREQUENCY',
                        oldIdentityCode,
                    ...
                    async: false,
                    success: function (res) {
                        console.log(`{{ url('temporaryTask/production/sub/autoBindEntireInstance') }} success:`, res);
                        location.reload();
                    },
                    error: function (err) {
                        console.log(`{{ url('temporaryTask/production/sub/autoBindEntireInstance') }} fail:`, err);
                        if (err.status === 401) location.href = "{{ url('login') }}";
                        alert(err['responseJSON']['message']);
                    }
                });
