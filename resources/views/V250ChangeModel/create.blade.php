...
@section('script')
    <script>
        ...
        function fnStore() {
            ...
            $.ajax({
                url: `{{ url('v250ChangeModel') }}?type={{ request('type') }}`,
                type: 'POST',
                data: data,
                async: true,
                success: function (res) {
                    console.log(`{{ url('v250ChangeModel') }}?type={{ request('type') }} success:`, res);
                    location.href = `/v250ChangeModel/${res.data.task_order.serial_number}/edit?type={{ request('type') }}`;
                },
                error: function (err) {
                    console.log(`{{ url('v250ChangeModel') }}?type={{ request('type') }} fail:`, err);
