                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-condensed">
                        ...
                        @foreach($tempTaskSubOrderEntireInstances as $entireInstance)
                            <tr id="tr_{{ @$entireInstance->id }}" class="{{ $entireInstance->in_scan ? 'bg-success' : '' }}">
                                <td>{{ @$entireInstance->OldEntireInstance->identity_code }}</td>
                                <td>{{ @$entireInstance->OldEntireInstance->model_name }}</td>
                                <td>
                                    {{ @$entireInstance->OldEntireInstance->maintain_location_code }}
                                    {{ @$entireInstance->OldEntireInstance->crossroad_number }}
                                </td>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body form-horizontal">
                        <form id="frmWarehouseIn">
                            ...
                            <input type="hidden" name="temp_task_id" value="{{ $tempTaskSubOrder->temp_task_id }}">
...
@section('script')
    <script>
        ...
        $(function () {
            ...
            $dpProcessedData_WarehouseOut.datepicker(datepickerOption);  // 入所日期
        ...
        function fnWarehouse(type) {
            ...
            $.ajax({
                url: `{{ url('tempTaskSubOrder',$tempTaskSubOrder->id) }}/warehouse?type=HIGH_FREQUENCY&direction=IN`,
                type: 'post',
                data: data,
                async: true,
                success: function (res) {
                    console.log(`{{ url('tempTaskSubOrder',$tempTaskSubOrder->id) }}/warehouse?type=HIGH_FREQUENCY&direction=IN success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('tempTaskSubOrder',$tempTaskSubOrder->id) }}/warehouse?type=HIGH_FREQUENCY&direction=IN fail:`, err);
                    if (err.status === 401) location.href = "{{ url('login') }}";
                    alert(err['responseJSON']['msg']);
                }
            });
