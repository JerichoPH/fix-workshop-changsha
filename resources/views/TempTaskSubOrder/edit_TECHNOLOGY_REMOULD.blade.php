                    ...
                    <div class="box-body">
                        <div class="table-responsive">
                            <div class="btn-group btn-group-sm pull-right">
                                ...
                                <a href="{{ url('tempTaskSubOrder',$tempTaskSubOrder->id) }}/bindEntireInstance?type=TECHNOLOGY_REMOULD" class="btn btn-default btn-flat"><i class="fa fa-link">&nbsp;</i>绑定设备</a>
                                ...
                                <a href="{{ url('tempTaskSubOrder',$tempTaskSubOrder->id) }}/warehouse?type=TECHNOLOGY_REMOULD&direction=IN" class="btn btn-flat btn-default"><i class="fa fa-sign-in">&nbsp;</i>添加入所单</a>
                                <a href="{{ url('tempTaskSubOrder',$tempTaskSubOrder->id) }}/warehouse?type=TECHNOLOGY_REMOULD&direction=OUT" class="btn btn-flat btn-default"><i class="fa fa-sign-out">&nbsp;</i>添加出所单</a>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body form-horizontal">
                        <form id="frmSearch">
                            ...
                            <input type="hidden" name="type" value="TECHNOLOGY_REMOULD_OLD">
                            <input type="hidden" name="lock_name" value="TECHNOLOGY_REMOULD">
                            <div class="table-responsive">
                                <table class="table table-striped table-hover table-condensed" id="table_Search">
                                    <thead>
                                    <tr>
                                        <th><input type="checkbox" name="" id="chkCheckAll2" onchange="fnCheckAll2(this)"></th>
                                        <th>唯一编号</th>
                                        <th>型号</th>
                                        <th>车站</th>
                                        <th>上道位置</th>
...
@section('script')
    <script>
        ...
        function fnCheckAll2(obj) {
            console.log('ok');
            $.each($('.entire-instance-identity-code_Search'), function (index, item) {
                if (!item.disabled) $(item).prop('checked', obj.checked);
            });
        }
         ...
         * 给搜索设备模态框的多选框增加多选监听
         */
        function onEntireInstanceIdentityCode_Search() {
            $('.entire-instance-identity-code_Search').on('change', function () {
                let all = 0;
                let checked = 0;
                ...
                $chkCheckAll2.prop('checked', checked === all);
            });
        }
        ...
        function fnFillTable_Search($table, data = []) {
            ...
            onEntireInstanceIdentityCode_Search();  // 重新给模态框中的多选框添加监听
        }
        ...
        $(function () {
            ...
            if (document.getElementById('table')) {
                $table.DataTable({
                    ...
                    aLengthMenu: [15, 50, 100, 200],  // 分页下拉框选项
                    processing: true,
                    language: {
                        processing: '加载中……',
                        sInfoFiltered: "从_MAX_中过滤",
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $.each(entireInstances, function (index, entireInstance) {
                        ...
                        let tableDatum = {
                            chk_input: `<input type="checkbox" id="entireInstanceIdentityCode_Search_${identity_code}" class="entire-instance-identity-code_Search" name="entireInstanceIdentityCode_Search[]" value="${identity_code}"/>`,
                            identity_code,
                            model_name,
                            maintain_station_name,
