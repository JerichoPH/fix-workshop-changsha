...
{{--            <li><a href="{{ url('tempTaskSubOrder',$tempTaskSubOrder->id) }}/edit"><i class="fa fa-users">&nbsp;</i>新站任务</a></li>--}}
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body form-horizontal">
                        <form id="frmWarehouseOut">
                            <input type="hidden" name="processor_id" value="{{ session('account.id') }}">
                            <input type="hidden" name="type" value="NEW_STATION">
...
@section('script')
    <script>
        ...
        function fnWarehouse(type) {
            ...
            $.ajax({
                url: `{{ url('tempTaskSubOrder',$tempTaskSubOrder->id) }}/warehouse?type=NEW_STATION&direction=OUT`,
                type: 'post',
                data: data,
                async: true,
                success: function (res) {
                    console.log(`{{ url('tempTaskSubOrder',$tempTaskSubOrder->id) }}/warehouse?type=NEW_STATION&direction=OUT success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('tempTaskSubOrder',$tempTaskSubOrder->id) }}/warehouse?type=NEW_STATION&direction=OUT fail:`, err);
                    if (err.status === 401) location.href = "{{ url('login') }}";
                    alert(err['responseJSON']['msg']);
                }
            });
