...
{{--                            <a href="{{ url('repairBase/newInOrder') }}?page={{ request('page',1) }}" class="btn btn-flat btn-default"><i class="fa fa-arrow-left">&nbsp;</i>返回</a>--}}
                            ...
                            <a href="{{ url('repairBase/newInOrder/mission') }}?page={{ request('page',1) }}&date={{ $new_in_order->created_at->format('Y-m') }}" class="btn btn-flat btn-default"><i class="fa fa-wrench">&nbsp;</i>分配检修任务</a>
                            <a href="javascript:" class="btn btn-flat btn-default" onclick="fnOut()"><i class="fa fa-sign-out">&nbsp;</i>查看/创建出所任务</a>
                            <a href="{{ url('repairBase/newInOrder/printLabel', $new_in_order->serial_number) }}?direction=IN" class="btn btn-flat btn-default" target="_blank"><i class="fa fa-print">&nbsp;</i>打印唯一编号</a>
                ...
                <div class="box box-solid">
                    ...
                    <div class="box-body">
                        <div class="input-group">
                            ...
                            <input type="text" name="no" id="txtNo" class="form-control" onkeydown="if(event.keyCode===13){fnAddScan(this.value)}" {{ array_flip(\App\Model\RepairBaseBreakdownOrder::$STATUSES)[$new_in_order->status] === 'DONE' ? 'disabled' : ''}}>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-condensed table-striped">
                                <thead>
...
@section('script')
    <script>
        ...
        function fnAddScan(identityCode) {
            $.ajax({
                ...
                success: function (res) {
                    console.log(`{{ url('repairBase/newInOrder/scanEntireInstances') }} success:`, res);
                    let html = '';
                ...
                error: function (err) {
                    console.log(`{{ url('repairBase/newInOrder/scanEntireInstances') }} fail:`, err);
        ...
        function fnDone() {
            $.ajax({
                url: `{{ url('repairBase/newInOrder/done',$new_in_order->serial_number) }}?direction=IN`,
                type: 'put',
        ...
        function fnOut() {
            $.ajax({
                url: `{{ url('repairBase/newInOrder') }}`,
                type: 'post',
                data: {sn: '{{ $new_in_order->serial_number }}', direction: 'OUT'},
                async: true,
                success: function (res) {
                    console.log(`{{ url('repairBase/newInOrder') }} success:`, res);
                    location.href = res['return_url'];
                },
                error: function (err) {
                    console.log(`{{ url('repairBase/newInOrder') }} fail:`, err);
                    if (err.status === 401) location.href = "{{ url('login') }}";
                    alert(err['responseJSON']['message']);
                }
            });
        ...
        function fnWarehouse() {
            $.ajax({
                url: `{{ url('repairBase/newInOrder/warehouse',$new_in_order->serial_number) }}?direction=IN`,
                type: 'post',
                data: {},
                async: true,
                success: function (res) {
                    console.log(`{{ url('repairBase/newInOrder/warehouse',$new_in_order->serial_number) }}?direction=IN success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('repairBase/newInOrder/warehouse',$new_in_order->serial_number) }}?direction=IN fail:`, err);
                    if (err.status === 401) location.href = "{{ url('login') }}";
                    alert(err['responseJSON']['message']);
                }
            });
