...
{{--                <li><a href="{{ url('repairBase/newInOrder') }}?direction=OUT">出所计划列表</a></li>--}}
{{--            @else--}}
{{--                <li><a href="{{ url('repairBase/newInOrder') }}?direction=IN">入所计划列表</a></li>--}}
{{--            @endif--}}
...
{{--        </ol>--}}
    ...
    <section class="content">
        ...
        <div class="box box-solid">
            <div class="box-header">
                <h3 class="box-title">新购{{ request('direction','IN') == 'IN' ? '入所' : '出所' }}列表</h3>
                {{--右侧最小化按钮--}}
                <div class="pull-right btn-group btn-group-sm">
                    @if(request('direction','IN') == 'IN')
                        <a href="{{ url('repairBase/buyIn/create') }}" class="btn btn-flat btn-success"><i class="fa fa-plus">&nbsp;</i>新建</a>
                    @endif
                </div>
            </div>
            <div class="box-body table-responsive">
                <div class="table-responsive">
                    <table class="table table-hover table-condensed" id="table">
                        ...
                        <tbody>
                        @foreach($new_in_orders as $new_in_order)
                            <tr>
                                <td>
                                    <a href="{{ url('repairBase/newInOrder',$new_in_order['serial_number']) }}?page={{ request('page',1) }}&scene_workshop_code={{ request('scene_workshop_code') }}&station_code={{ request('station_code') }}&direction={{ request('direction','IN') }}&created_at={{ request('created_at') }}">
                                        {{ $new_in_order->SceneWorkshop ? $new_in_order->SceneWorkshop->name : '' }}
                                        {{ $new_in_order->Station ? $new_in_order->Station->name : '' }}
                                    </a>
                                </td>
                                <td>{{ $new_in_order->created_at->format('Y-m') }}</td>
                                <td>{{ $new_in_order->status }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
            ...
            @if($new_in_orders->hasPages())
                <div class="box-footer">
                    {{ $new_in_orders->appends([
                                                        'page'=>request('page',1),
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">新建新购入所计划</h4>
                    </div>
        ...
        function fnCreate() {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    location.href = `/repairBase/newInOrder/create/${res['new_serial_number']}?page={{ request('',1) }}&direction=IN`;
                },
                error: function (err) {
