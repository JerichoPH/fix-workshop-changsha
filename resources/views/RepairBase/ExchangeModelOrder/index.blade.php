...
{{--                <li><a href="{{ url('repairBase/exchangeModelOrder') }}?direction=OUT">出所计划列表</a></li>--}}
{{--            @else--}}
{{--                <li><a href="{{ url('repairBase/exchangeModelOrder') }}?direction=IN">入所计划列表</a></li>--}}
{{--            @endif--}}
...
{{--        </ol>--}}
    ...
    <section class="content">
        ...
        <div class="box box-solid">
            <div class="box-header">
                <h3 class="box-title">更换设备{{ request('direction','IN') == 'IN' ? '入所' : '出所' }}列表</h3>
                {{--右侧最小化按钮--}}
                <div class="pull-right btn-group btn-group-sm">
                    @if(request('direction','IN') == 'IN')
            ...
            <div class="box-body table-responsive">
                <div class="table-responsive">
                    <table class="table table-hover table-condensed" id="table">
                        ...
                        <tbody>
                        @foreach($exchange_model_orders as $exchange_model_order)
                            <tr>
                                <td>
                                    <a href="{{ url('repairBase/exchangeModelOrder',$exchange_model_order['serial_number']) }}?page={{ request('page',1) }}&scene_workshop_code={{ request('scene_workshop_code') }}&station_code={{ request('station_code') }}&direction={{ request('direction','IN') }}&created_at={{ request('created_at') }}">
                                ...
                                <td>{{ $exchange_model_order->created_at->format('Y-m') }}</td>
                                <td>{{ $exchange_model_order->status }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
            ...
            @if($exchange_model_orders->hasPages())
                <div class="box-footer">
                    {{ $exchange_model_orders->appends([
                                                        'page'=>request('page',1),
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">新建更换设备入所计划</h4>
                    </div>
        ...
        function fnCreate() {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    location.href = `/repairBase/exchangeModelOrder/create/${res['new_serial_number']}?page={{ request('',1) }}&direction=IN`;
                },
                error: function (err) {
