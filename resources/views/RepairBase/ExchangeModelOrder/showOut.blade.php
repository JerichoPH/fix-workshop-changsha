...
{{--            <li><a href="{{ url('repairBase/exchangeModelOrder') }}?page={{ request('page',1) }}&direction={{ request('direction','OUT') }}"><i class="fa fa-users">&nbsp;</i>更换设备管理</a></li>--}}
    ...
    <section class="content">
        ...
        <div class="box box-solid">
            <div class="box-header">
                <div class="row">
                    <div class="col-md-8">
                        <h3 class="box-title">更换设备出所计划详情</h3>
                    </div>
                    <div class="col-md-4">
                        <!--右侧最小化按钮-->
                        <div class="input-group pull-right">
                            <div class="input-group-btn"><a href="{{ url('repairBase/exchangeModelOrder') }}?direction=OUT&page={{ request('page',1) }}" class="btn btn-flat btn-default"><i class="fa fa-arrow-left">&nbsp;</i>返回</a></div>
                            <div class="input-group-addon">唯一编号</div>
                            <input type="text" name="identity_code" id="txtIdentityCode" class="form-control" onkeydown="if(event.keyCode===13){fnOutScan(this.value);}" {{ array_flip(\App\Model\RepairBaseBreakdownOrder::$STATUSES)[$exchange_model_order->status] === 'DONE' ? 'disabled' : ''}}>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="btn-group btn-group-sm pull-right">
                            ...
                            @endif
                            <a href="{{ url('repairBase/exchangeModelOrder/printLabel',$exchange_model_order->serial_number) }}?direction=OUT" class="btn btn-primary btn-flat"><i class="fa fa-print">&nbsp;</i>打印标签</a>
...
@section('script')
    <script>
        ...
        let fnWarehouse = () => {
            $.ajax({
                url: `{{ url('repairBase/exchangeModelOrder/warehouse') }}?direction=OUT`,
                type: 'post',
                data: {
                    exchangeModelOrderSn:'{{ $exchange_model_order->serial_number }}',
                },
                async: true,
                success: function (res) {
                    console.log(`{{ url('repairBase/exchangeModelOrder/warehouse') }} success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('repairBase/exchangeModelOrder/warehouse') }} fail:`, err);
                    if (err.status === 401) location.href = "{{ url('login') }}";
                    alert(err['responseJSON']['message']);
                }
            });
        ...
        function fnDone() {
            $.ajax({
                url: `{{ url('repairBase/exchangeModelOrder/done',$exchange_model_order->serial_number) }}?direction=OUT`,
                type: 'put',
                data: {},
                async: true,
                success: function (res) {
