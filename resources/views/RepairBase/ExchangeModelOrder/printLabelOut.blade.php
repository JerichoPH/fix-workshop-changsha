...
{{--            <li><a href="{{ url('repairBase/exchangeModelOrder',$out_sn) }}?direction=OUT">更换设备出所计划详情</a></li>--}}
    ...
    <section class="content">
        ...
        <div class="box box-solid">
            <div class="box-header">
                <h3 class="box-title">更换设备出所设备列表</h3>
                <!--右侧最小化按钮-->
                <div class="pull-right btn-group btn-group-sm">
                    <a href="{{ url('repairBase/exchangeModelOrder',$out_sn) }}?direction=OUT" class="btn btn-flat btn-default"><i class="fa fa-arrow-left">&nbsp;</i>返回</a>
...
@section('script')
    <script>
        ...
        function fnAutoBindEntireInstances() {
            $.ajax({
                url: `{{ url('repairBase/exchangeModelOrder/autoBindEntireInstances') }}`,
                type: 'post',
                data: {outSn: '{{ $out_sn }}'},
                async: false,
                success: function (res) {
                    console.log(`{{ url('repairBase/exchangeModelOrder/autoBindEntireInstances') }} success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('repairBase/exchangeModelOrder/autoBindEntireInstances') }} fail:`, err);
        ...
        function fnDeleteBindEntireInstances() {
            $.ajax({
                url: `{{ url('repairBase/exchangeModelOrder/bindEntireInstances') }}`,
                type: 'delete',
                data: {outSn: '{{ $out_sn }}'},
                async: false,
                success: function (res) {
                    console.log(`{{ url('repairBase/exchangeModelOrder/bindEntireInstances') }} success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('repairBase/exchangeModelOrder/bindEntireInstances') }} fail:`, err);
                    if (err.status === 401) location.href = "{{ url('login') }}";
                    alert(err['responseJSON']['message']);
                }
            });
        ...
        function fnBindEntireInstance(oldIdentityCode, newIdentityCode) {
            if (newIdentityCode) {
                $.ajax({
                    url: `{{ url('repairBase/exchangeModelOrder/bindEntireInstance') }}`,
                    type: 'post',
                    data: {oldIdentityCode, newIdentityCode, outSn: '{{ $out_sn }}'},
                    async: false,
                    success: function (res) {
                        console.log(`{{ url('repairBase/exchangeModelOrder/bindEntireInstance') }} success:`, res);
                        location.reload();
                    },
                    error: function (err) {
                        console.log(`{{ url('repairBase/exchangeModelOrder/bindEntireInstance') }} fail:`, err);
                        if (err.status === 401) location.href = "{{ url('login') }}";
                        alert(err['responseJSON']['message']);
                    }
                });
        ...
        $('.select-bind-entire-instance').on('change', function () {
            ...
            if ($(this).is(':checked')) {
                // 绑定
                $.ajax({
                    url: `{{ url('repairBase/exchangeModelOrder/autoBindEntireInstance') }}`,
                    type: 'post',
                    data: {oldIdentityCode, outSn: '{{ $out_sn }}'},
                    async: false,
                    success: function (res) {
                        console.log(`{{ url('repairBase/exchangeModelOrder/autoBindEntireInstance') }} success:`, res);
                        location.reload();
                    },
                    error: function (err) {
                        console.log(`{{ url('repairBase/exchangeModelOrder/autoBindEntireInstance') }} fail:`, err);
                        if (err.status === 401) location.href = "{{ url('login') }}";
                        alert(err['responseJSON']['message']);
                    }
                });
