...
{{--            <li><a href="{{ url('repairBase/highFrequencyOrder',$out_sn) }}?direction=OUT">高频修出所计划详情</a></li>--}}
        ...
        <div class="box box-solid">
            <div class="box-header">
                ...
                <div class="pull-right btn-group btn-group-sm">
                    <a href="{{ url('repairBase/highFrequencyOrder',$out_sn) }}?direction=OUT" class="btn btn-flat btn-default"><i class="fa fa-arrow-left">&nbsp;</i>返回</a>
...
@section('script')
    <script>
        ...
        function fnAutoBindEntireInstances() {
            $.ajax({
                url: `{{ url('repairBase/highFrequencyOrder/autoBindEntireInstances') }}`,
                type: 'post',
                data: {outSn: '{{ $out_sn }}'},
                async: false,
                success: function (res) {
                    console.log(`{{ url('repairBase/highFrequencyOrder/autoBindEntireInstances') }} success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('repairBase/highFrequencyOrder/autoBindEntireInstances') }} fail:`, err);
        ...
        function fnDeleteBindEntireInstances() {
            $.ajax({
                url: `{{ url('repairBase/highFrequencyOrder/bindEntireInstances') }}`,
                type: 'delete',
                data: {outSn: '{{ $out_sn }}'},
                async: false,
                success: function (res) {
                    console.log(`{{ url('repairBase/highFrequencyOrder/bindEntireInstances') }} success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('repairBase/highFrequencyOrder/bindEntireInstances') }} fail:`, err);
                    if (err.status === 401) location.href = "{{ url('login') }}";
                    alert(err['responseJSON']['message']);
                }
            });
        ...
        $('.select-bind-entire-instance').on('change', function () {
            ...
            if ($(this).is(':checked')) {
                // 绑定
                $.ajax({
                    url: `{{ url('repairBase/highFrequencyOrder/autoBindEntireInstance') }}`,
                    type: 'post',
                    data: {oldIdentityCode, outSn: '{{ $out_sn }}'},
                    async: false,
                    success: function (res) {
                        console.log(`{{ url('repairBase/highFrequencyOrder/autoBindEntireInstance') }} success:`, res);
                        location.reload();
                    },
                    error: function (err) {
                        console.log(`{{ url('repairBase/highFrequencyOrder/autoBindEntireInstance') }} fail:`, err);
                        if (err.status === 401) location.href = "{{ url('login') }}";
                        alert(err['responseJSON']['message']);
                    }
                });
