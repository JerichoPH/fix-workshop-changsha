...
{{--            <li><a href="{{ url('repairBase/highFrequencyOrder') }}?page={{ request('page',1) }}&direction={{ request('direction','OUT') }}"><i class="fa fa-users">&nbsp;</i>高频修管理</a></li>--}}
    ...
    <section class="content">
        ...
        <div class="box box-solid">
            <div class="box-header">
                <div class="row">
                    <div class="col-md-8">
                        <h3 class="box-title">高频修出所计划详情</h3>
                    </div>
                    <div class="col-md-4">
                        <!--右侧最小化按钮-->
                        <div class="input-group pull-right">
                            <div class="input-group-btn"><a href="{{ url('repairBase/highFrequencyOrder') }}?direction=OUT&page={{ request('page',1) }}" class="btn btn-flat btn-default"><i class="fa fa-arrow-left">&nbsp;</i>返回</a></div>
                            <div class="input-group-addon">唯一编号</div>
                            <label for="txtIdentityCode" style="display: none;"></label>
                            <input type="text" name="identity_code" id="txtIdentityCode" class="form-control" onkeydown="if(event.keyCode===13){fnOutScan(this.value);}" {{ array_flip(\App\Model\RepairBaseHighFrequencyOrder::$STATUSES)[$high_frequency_order->status] === 'DONE' ? 'disabled' : ''}}>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="box-body">
                ...
                <div class="table-responsive">
                    <div class="btn-group">
                        ...
                        @endif
                        <a href="{{ url('repairBase/highFrequencyOrder/printLabel',$high_frequency_order->serial_number) }}?direction=OUT" class="btn btn-primary btn-flat"><i class="fa fa-link">&nbsp;</i>打印标签</a>
...
@section('script')
    <script>
        ...
        let fnWarehouse = () => {
            $.ajax({
                url: `{{ url('repairBase/highFrequencyOrder/warehouse',$high_frequency_order->serial_number) }}?direction=OUT`,
                type: 'post',
                data: {},
                async: true,
                success: function (res) {
                    console.log(`{{ url('repairBase/highFrequencyOrder/warehouse',$high_frequency_order->serial_number) }} success:`, res);
                    location.reload();
                },
                error: function (err) {
                    console.log(`{{ url('repairBase/highFrequencyOrder/warehouse',$high_frequency_order->serial_number) }} fail:`, err);
                    if (err.status === 401) location.href = "{{ url('login') }}";
                    alert(err['responseJSON']['message']);
                }
            });
        ...
        function fnDone() {
            $.ajax({
                url: `{{ url('repairBase/highFrequencyOrder/done') }}?direction=OUT`,
                type: 'put',
                data: {
                    hightFrequencyOrderSn: '{{ $high_frequency_order->serial_number }}',
                },
                async: true,
                success: function (res) {
