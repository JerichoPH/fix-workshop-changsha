...
{{--                    <a href="{{url('rbac/role')}}?page={{request('page')}}" class="btn btn-default pull-left btn-lg btn-flat">返回</a>--}}
                    <a href="#" onclick="javascript :history.back(-1);" class="btn btn-default pull-left btn-lg btn-flat">返回</a>
                    <a href="javascript:" onclick="fnUpdate()" class="btn btn-warning pull-right btn-lg btn-flat">保存</a>
                </div>
            </form>
        </div>
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                ...
                <div class="box-tools pull-right">
                    <a href="{{url('rbac/permission/create')}}" class="btn btn-box-tool">新建权限</a>
                </div>
            </div>
            <form class="form-horizontal" id="frmBindPermission">
                ...
                @endforeach
                <div class="box-footer">
                    <a href="javascript:" onclick="fnBindPermissions('{{$role->id}}')" class="btn btn-primary pull-right btn-flat"><i class="fa fa-check">&nbsp;</i>确定</a>
                </div>
            </form>
        </div>
    </section>
...
@section('script')
    <script>
        ...
        fnUpdate = function () {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    alert(response);
                    location.href = "{{url('rbac/role')}}?page{{request('page',1)}}";
                },
        ...
        fnBindPermissions = function (roleId) {
            $.ajax({
                url: `{{url('rbac/roleBindPermissions')}}/${roleId}`,
                type: "post",
