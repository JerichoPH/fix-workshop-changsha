                ...
                <div class="box box-solid">
                    ...
                    <form class="form-horizontal" id="frmUpdate">
                        ...
                        <div class="box-footer">
                            {{--<a href="{{url('rbac/menu')}}?page={{request('page',1)}}" class="btn btn-default pull-left btn-flat"><i class="fa fa-arrow-left">&nbsp;</i>返回</a>--}}
            ...
            <div class="col-md-6">
                <div class="box box-solid">
                    ...
                    <form id="frmBindRoles">
                                    ...
                                    @foreach($roles as $chunk)
                                        <tr>
                                            @foreach($chunk as $role)
                                                <td><label><input type="checkbox" name="role_ids[]" value="{{ $role->id }}" {{ in_array($role->id,$roleIds) ? 'checked' : '' }}>{{ $role->name }}</label></td>
                                            @endforeach
                                        </tr>
                                    @endforeach
                                </table>
                        ...
                        <div class="box-footer">
                            <a href="javascript:" onclick="fnBindRoles('{{$menu->id}}')" class="btn btn-primary pull-right btn-flat"><i class="fa fa-link">&nbsp;</i>绑定</a>
                        </div>
                    </form>
                </div>
            </div>
...
@section('script')
    <script>
        ...
        fnUpdate = function () {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    location.href = "{{url('rbac/menu')}}?page{{request('page',1)}}";
                },
        ...
        fnBindRoles = function (menuId) {
            $.ajax({
                url: "{{url('rbac/menuBindRoles')}}/" + menuId,
                type: "post",
