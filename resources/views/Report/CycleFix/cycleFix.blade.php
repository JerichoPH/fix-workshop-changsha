...
@section('script')
    <script>
        ...
        function fnMakeCycleFixTable(dateType = null, date = null) {
            ...
            $.ajax({
                url: `{{ url('report/cycleFix/reportForCategoriesWithYear') }}`,
                type: 'get',
                data: {year},
                async: true,
                success: function (res) {
                    let {year, statistics, missions} = res;

                    // 格式化数据
                    let tmp = [];
                    $.each(statistics, function (month, statistic) {
                        $.each(statistic, function (cu, c) {
                            if (!categories.hasOwnProperty(cu)) categories[cu] = c['name'];
                            if (!categoriesAsFlip.hasOwnProperty(c['name'])) categoriesAsFlip[c['name']] = cu;
                            if (!tmp.hasOwnProperty(cu)) tmp[cu] = {name: c['name'], statistics: {}};
                            if (!tmp[cu]['statistics'].hasOwnProperty(month)) tmp[cu]['statistics'][month] = null;
                            tmp[cu]['statistics'][month] = c['statistics'];
                        });
                    });

                    // 生成表头
                    theadHtml = '<tr><th>种类/时间</th><th>合计</th>';
                    for (let i = 1; i < 13; i++) {
                    ...
                    $.each(categories, function (cu, cn) {
                        ...
                        tbodyHtml += `<tr onclick="fnToCycleFixWithCategory('${cu}')"><td>${cn}</td>`;
                        let monthHtml = '';

                        for (let i = 1; i < 13; i++) {
                            let month = strPad(i);
                            countColPlan += tmp.hasOwnProperty(cu) ? (tmp[cu]['statistics'].hasOwnProperty(month) ? tmp[cu]['statistics'][month]['plan_device_count'] : 0) : 0;
                            countColFixed += tmp.hasOwnProperty(cu) ? (tmp[cu]['statistics'].hasOwnProperty(month) ? tmp[cu]['statistics'][month]['fixed_device_count'] : 0) : 0;
                            let plan = tmp.hasOwnProperty(cu) ? (tmp[cu]['statistics'].hasOwnProperty(month) ? tmp[cu]['statistics'][month]['plan_device_count'] : 0) : 0;
                            let mission = missions.hasOwnProperty(cu) ? (missions[cu].hasOwnProperty(month) ? missions[cu][month]['aggregate'] : 0) : 0;
                            let fixed = tmp.hasOwnProperty(cu) ? (tmp[cu]['statistics'].hasOwnProperty(month) ? tmp[cu]['statistics'][month]['fixed_device_count'] : 0) : 0;
                            monthHtml += `<td><span>计划：${plan}</span><br>
<span>任务：${mission}</span><br>
<span>完成：${fixed}</span></td>
`;
                        }
                        tbodyHtml += `<td><span>计划：${countColPlan}</span><br><span>任务：</span><br><span>完成：${countColFixed}</span></td>` + monthHtml + '</tr>';
                    });
...
检修总计：${params[2].value}`
                    ...
                    cycleFixECharts.on('click', function (params) {
                        let categoryUniqueCode = categoriesAsFlip[params['name']] ? categoriesAsFlip[params['name']] : null;
                        fnToCycleFixWithCategory(categoryUniqueCode);
                    });
                },
                error: function (err) {
                    {{--console.log(`{{ url('reportData')  }} fail:`, err);--}}
         ...
         */
        function fnToCycleFixWithCategory(categoryUniqueCode = null) {
            if (categoryUniqueCode) location.href = `/report/cycleFixWithCategory/${categoryUniqueCode}?date={{ request('date', date('Y')) }}`;
        }
    </script>
@endsection
