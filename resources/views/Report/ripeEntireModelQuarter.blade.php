...
@section('content')
    ...
    <section class="content-header">
        <h1>
            一次过检
            <small>{{$current_entire_model_name}} 季度</small>
        </h1>
{{--        <ol class="breadcrumb">--}}
{{--            <li><a href="/"><i class="fa fa-dashboard"></i> 主页</a></li>--}}
{{--            <li><a href="/report/ripeCategoryQuarter?year={{request('year',date('Y'))}}&quarter={{request('quarter','1季度')}}">一次过检 {{$current_category_name}}</a></li>--}}
...
@section('script')
    <script>
        ...
        let subModels = JSON.parse('{!! $sub_model_as_json !!}');
...
返修率：${params[1].value > 0 ? ((params[1].value / params[0].value) * 100).toFixed(3) : 0}%`;
        ...
        let fnCheckDateType = dateType => {
            location.href = `/report/ripeEntireModel${dateType}/{{$current_entire_model_unique_code}}?year={{request('year',date('Y'))}}&quarter={{request('quarter','1季度')}}`;
        };
        ...
        let fnToSubModel = subModelName => {
            location.href = `/report/ripeSubModelQuarter/${subModels[subModelName]}?year=${$('#selYear').val()}&quarter=${$('#selQuarter').val()}`;
        };
        ...
        let fnToQualityEntireInstance = subModelName => {
            ...
            location.href = `/report/qualityEntireInstance?category_unique_code=${categoryUniqueCode}&entire_model_unique_code=${entireModelUniqueCode}&sub_model_unique_code=${subModelUniqueCode}&year=${$('#selYear').val()}&type=quarter&quarter=${$('#selQuarter').val()}`;
        };
        ...
        let fnToRipeEntireInstance = subModelName => {
            ...
            let categoryUniqueCode = subModelUniqueCode.substr(0, 3);
            let entireModelUniqueCode = subModelUniqueCode.substr(0, 5);
            location.href = `/report/ripeEntireInstance?category_unique_code=${categoryUniqueCode}&entire_model_unique_code=${entireModelUniqueCode}&sub_model_unique_code=${subModelUniqueCode}&year=${$('#selYear').val()}&type=quarter&quarter=${$('#selQuarter').val()}`;
        };
    </script>
@endsection
