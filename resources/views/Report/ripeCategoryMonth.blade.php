...
@section('content')
    ...
    <section class="content-header">
        <h1>
            一次过检
            <small>{{$current_category_name}} 月度</small>
        </h1>
{{--        <ol class="breadcrumb">--}}
{{--            <li><a href="/"><i class="fa fa-dashboard"></i> 主页</a></li>--}}
{{--            <li><a href="/report/ripeMonth?year={{request('year',date('Y'))}}&month={{request('month',date('m'))}}">一次过检 全部种类</a></li>--}}
...
返修率：${params[1].value > 0 ? ((params[1].value / params[0].value) * 100).toFixed(3) : 0}%`;
        ...
        function fnCheckDateType(dateType) {
            location.href = `/report/ripeCategory${dateType}/{{$current_category_unique_code}}?year={{request('year',date('Y'))}}&month={{request('month',date('m'))}}`;
        }
        ...
        function fnToEntireModel(entireModelName) {
            location.href = `/report/ripeEntireModelMonth/${entireModels[entireModelName]}?year=${$('#selYear').val()}&month=${$('#selMonth').val()}`;
        }
