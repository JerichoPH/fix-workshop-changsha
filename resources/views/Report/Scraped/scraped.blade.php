    ...
    <script>
        ...
        function fnMakeScrapedChart() {
            ...
            $.each(scrapedDevicesAsKind, function (cu, item) {
                ...
                series[0]['data'].push(propertyDevicesAsKind[cu]['statistics']['device_total'])
...
超期使用率：${params[0].value > 0 ? ((params[1].value / params[0].value) * 100).toFixed(2) : 0}%`;
        ...
        function fnMakeScrapedTable() {
            let html = '';
            $.each(scrapedDevicesAsKind, function (cu, item) {
                html += `<tr><td><a href="/report/scrapedWithCategory/${cu}">${categories[cu]}</a></td>`;
                html += `<td><a href="/report/propertySubModel?category_unique_code=${cu.substr(0, 3)}">${propertyDevicesAsKind[cu]['statistics']['device_total']}</a></td>`;
                html += `<td><a href="/report/scrapedWithSubModel/${cu}">${item['statistics']['scraped_device_count']}</a></td>`;
                html += `<td>${((item['statistics']['scraped_device_count'] / propertyDevicesAsKind[cu]['statistics']['device_total']) * 100).toFixed(2)}%</td></tr>`;
            });
