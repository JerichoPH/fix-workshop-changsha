...
{{--            <li class="active">临时检修任务 所有种类</li>--}}
{{--        </ol>--}}
    </section>
                    ...
                    <div class="box-body form-horizontal">
                        <div class="table-responsive">
                            <table class="table table-hover table-condensed text-sm">
                                ...
                                <tbody>
                                @foreach($ttpm_mission_with_year['items'] as $category_name => $mission)
                                    <tr onclick="location.href='{{ url('/report/temporaryTask/production/main/withCategory',$ttpm_categories_flip[$category_name]) }}?year={{ request('year',date('Y')) }}'">
                                        <td>{{ $category_name }}</td>
                                        <td>
                                            <p class="text-sm">
                                                任务<br>
                                        ...
                                        <td>
                                            <p class="text-sm text-center">
                                                {{ $mission }}<br>
                                                ---<br>
                                                {{ isset($ttpm_fixed_with_year['items'][$category_name]) ? $ttpm_fixed_with_year['items'][$category_name] : 0 }}
                                            </p>
                                        </td>
                                        @for($i=0;$i<12;$i++)
                                            <td>
                                                <p class="text-sm text-center">
                                                    {{ isset($ttpm_mission_with_month[$i]['items'][$category_name]) ? $ttpm_mission_with_month[$i]['items'][$category_name] : 0 }}<br>
                                                    ---<br>
                                                    {{ isset($ttpm_fixed_with_month[$i]['items'][$category_name]) ? $ttpm_fixed_with_month[$i]['items'][$category_name] : 0 }}
                                                </p>
                                            </td>
                                        @endfor
                                    </tr>
...
@section('script')
    <script>
        ...
        let fnMakePlanAndFinishChart = () => {
            let cycleFixECharts = echarts.init(document.getElementById('echartsTemporaryTaskProductionMain'));
            let ttpmCategoryAsNames = [];
            let ttpmMissionWithYear = JSON.parse('{!! $ttpm_mission_with_year_as_json !!}')['items'];
            let ttpmFixedWithYear = JSON.parse('{!! $ttpm_fixed_with_year_as_json !!}')['items'];
...
检修总计：${params[2].value}`;
        ...
        function fnToCategory(categoryUniqueCode) {
            location.href = `/report/temporaryTask/production/main/withCategory/${categoryUniqueCode}?year={{$year}}`;
        }
    </script>
@endsection
