...
返修率：${params[1].value > 0 ? ((params[1].value / params[0].value) * 100).toFixed(3) : 0}%`;
        ...
        let fnToCategory = categoryName => {
            location.href = `/report/ripeCategoryMonth/${categories[categoryName]}?year=${$('#selYear').val()}&month=${$('#selMonth').val()}`;
        };
        ...
        let fnToQualityEntireInstance = categoryName => {
            location.href = `/report/qualityEntireInstance?category_unique_code=${categories[categoryName]}&year=${$('#selYear').val()}&type=month&month=${$('#selMonth').val()}`;
        };
        ...
        let fnToRipeEntireInstance = categoryName => {
            location.href = `/report/ripeEntireInstance?category_unique_code=${categories[categoryName]}&year=${$('#selYear').val()}&type=month&month=${$('#selMonth').val()}`;
        };
    </script>
@endsection
