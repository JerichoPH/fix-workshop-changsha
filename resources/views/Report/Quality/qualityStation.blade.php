    ...
    <section class="content">
        ...
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">质量报告 {{ $sceneWorkshopName }} {{ $stationName }} <small>供应商</small></h3>
...
@section('script')
    <script>
        ...
        function fnChangeQualityDateType(type) {
            location.href = `{{ url('report/qualityStation',$sceneWorkshopUniqueCode) }}/{{ $stationUniqueCode }}?qualityDateType=${type}`;
        }

        function fnSearch(qualityDate) {
            location.href = `{{ url('report/qualityStation',$sceneWorkshopUniqueCode) }}/{{ $stationUniqueCode }}?qualityDateType={{ $qualityDateType }}&qualityDate=${qualityDate}`;
        }
...
返修率：${params[1].value > 0 ? ((params[1].value / params[0].value) * 100).toFixed(4) : 0}%`;
            ...
            echartsQuality.on('click', function (params) {
                if (params.seriesName === '返修') {
                    {{--location.href = `{{ url('report/qualityEntireInstance') }}?factoryUniqueCode=${factoryNames[params.name]}&qualityDateType=${qualityDateType}&qualityDate=${qualityDate}&sceneWorkshopUniqueCode={{$sceneWorkshopUniqueCode}}&stationUniqueCode={{$stationUniqueCode}}`;--}}
                }
            });
        }
        ...
        function fnMakeQualityFactoryTable() {
            ...
            $.each(factories, function (code, name) {
                ...
                if (breakdownCount > 0) {
                    html += `<td><a href="{{ url('report/qualityEntireInstance') }}?factoryUniqueCode=${code}&qualityDateType=${qualityDateType}&qualityDate=${qualityDate}&sceneWorkshopUniqueCode={{$sceneWorkshopUniqueCode}}&stationUniqueCode={{$stationUniqueCode}}&type=breakdown">${breakdownCount}</a></td>`;
                } else {
